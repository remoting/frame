...
func DesEncryptECB(plainText []byte, key []byte, padding int) ([]byte, error) {
...
func DesDecryptECB(cipherText []byte, key []byte, padding int) ([]byte, error) {
...
func DesEncryptECBTriple(plainText []byte, key []byte, padding int) ([]byte, error) {
...
// The length of the `key` should be either 16 or 24 bytes.
func DesDecryptECBTriple(cipherText []byte, key []byte, padding int) ([]byte, error) {
	if len(key) != 16 && len(key) != 24 {
		return nil, errors.New("key length error")
	}
...
func DesEncryptCBCTriple(plainText []byte, key []byte, iv []byte, padding int) ([]byte, error) {
	if len(key) != 16 && len(key) != 24 {
...
func DesDecryptCBCTriple(cipherText []byte, key []byte, iv []byte, padding int) ([]byte, error) {
	if len(key) != 16 && len(key) != 24 {
		return nil, errors.New("key length invalid")
	}
